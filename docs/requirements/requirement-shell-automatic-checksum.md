...
This requirement is the **project Single Source of Truth** for **automatic companion-digest integrity** of the pomo POSIX shell tool: downloading a SHA-256 sidecar next to the install channel, verifying install/self-update downloads, and reporting the process **transparently** (companion **link**, expected **value**, and verification **result**).
...
| **In-repo publisher SSOT** | Companion file **`pomo.sha256`** (bare SHA-256 hex of `./pomo`) **MUST** ship next to the installable script for the release channel. |
| **Regenerate on change** | After every edit to `./pomo` that is published, regenerate `pomo.sha256` so automatic mode can match. |
...
| **Outside payload** | Pin is env/operator/CI for that process — **MUST NOT** be embedded inside `./pomo` as a self-hash of that file. |
...
1. Hash of `./pomo` stored **inside** `./pomo` as the verify target.  
...
1. Product root **`README.md` MUST** explain **automatic** checksum as the default: algorithm (SHA-256), companion URL pattern, in-repo `pomo.sha256`, match / mismatch / missing outcomes.  
...
| **Channel SSOT** | `SCRIPT_URL` default `https://raw.githubusercontent.com/${REPO_USER}/${REPO_NAME}/main/${APP_NAME}` → `https://raw.githubusercontent.com/Wilgat/pomo/main/pomo` |
| **Companion URL** | `${SCRIPT_URL}.sha256` → `https://raw.githubusercontent.com/Wilgat/pomo/main/pomo.sha256` |
| **In-repo companion** | `pomo.sha256` (bare 64-char hex) |
...
2. Embed the expected SHA-256 of `./pomo` **inside** `./pomo` as verification.  
...
5. Document same-origin `CHECKSUM=$(curl …/pomo.sha256)` as higher assurance than automatic companion verification.  
