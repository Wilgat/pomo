...
| Item | Value for pomo |
|------|------------------------|
| **Product / binary name** | `pomo` (`APP_NAME`, default `pomo`) |
...
| `start` | Type 0 domain | `pomo_start` | Start work phase (default 25m work / 5m break); optional name, minutes, `--break N`, `--persist` |
| `status` | Type 0 domain | `pomo_show_status` | Themed status + progress; auto work→break transition when work expires |
| `watch` | Type 0 domain | `pomo_watch` | Live refresh (not with `--json` / `--quiet`) |
| `skip` | Type 0 domain | `pomo_skip` | Skip phase; work skip counts toward daily stats |
| `stop` | Type 0 domain | `pomo_stop` | Complete work session (counts stats) unless `--force` |
| `kill` | Type 0 domain | `pomo_stop` (force) | Discard without counting |
...
| `--force` | Parsed by `app_main` → `FORCE=1` and `FORCE_REINSTALL=1`; used by install reinstall, self-update (incl. deliberate downgrade), uninstall confirm skip, and domain discard |
| `--persist` | Domain: persistent storage for pomodoro state |
| `--break N` | Domain: break duration minutes for `start` |
...
This requirement is satisfied for the pomo shell CLI when all of the following hold:
