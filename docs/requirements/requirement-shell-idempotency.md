...
This requirement is the **project Single Source of Truth** for **idempotency (re-run safety)** of state-changing operations in the **POSIX shell CLI** for pomo.
...
| Domain `start` (same name+mode) | Timer state present | **Fail closed** `already_running` (not overwrite) | — |
| Domain `status` / `list` / `stats` | N/A (read) | Safe to re-run | — |
| Domain `theme set` | Theme preference file | Re-set same theme is success overwrite of preference | Invalid theme fails closed |
...
| Re-run install-ensure / already-installed as this login | In-tool `sudo`; wrap `apt`/`dnf`; create a dedicated system user; recommend `sudo curl \| sh` |
...
**This requirement:** re-run safety stays this-login on this class; do not invent a root reinstall path because Termux was detected.
...
A state-changing shell change for pomo is **not done** if any of the following fail:
