...
This requirement is the **project Single Source of Truth** for how the pomo **POSIX shell CLI** behaves in **interactive** (human + TTY) versus **non-interactive** (automation, `curl | sh`, CI/CD, pipes, `--json` / often `--quiet`) environments.
...
#### Domain interaction (this project)

| Command / path | Interactive | Non-interactive / automation |
|----------------|-------------|------------------------------|
...
Mode-related work for pomo is **not done** if any of the following fail:
...
| **TP-CURL-01**…**08** local curl\|sh (`PM-ONLINE-CURL-INSTALL-TEST-PLAN` Core) | `tests/test_online_curl_install.sh` | have |
| **TP-CURL-04** hostile HOME pipe + direct | same (mold Core + **TP-U-03**) | have |
