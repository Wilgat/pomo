...
This requirement is the **project Single Source of Truth** for **modular function organization** of the pomo POSIX shell CLI.
...
Optional multi-file layout under `src/` for future authoring **MAY** exist only if a build or pack step still produces **one** installable artifact and this requirement is updated. Until then, `./pomo` remains the single shipped script.
...
Optional `ALIGNMENT` / `See` / “fully synchronized with” lines in **product source** (`./pomo`) **MUST** cite only **live** `docs/requirements/requirement-*.md` paths that exist on disk and appear in `docs/requirements/index.md`.
...
| **Single shipped script** | Repo root `./pomo` (~3500 lines, `#!/bin/sh`) |
| **`src/` directory** | **Not used** for runtime — single-file ship unit only |
| **Domain prefix `pomo_*`** | **In use** — pomodoro domain ops (`start` / `status` / `watch` / `skip` / `stop` / `kill` / `list` / `stats` / `theme`; work/break phases) |
...
| `util_` | `util_json_escape`, `util_sha256_file`, `util_fetch_remote_version`, `util_get_install_bin_path`, `util_backup`, `util_resolve_storage`, `util_ensure_writable_dir`, `util_resolve_volatile_root`, `util_get_current_shell`, `util_app_ident` (`GIT_BASH_DRIVE` with storage / Git Bash REQs) |
...
| `app_` | `app_about`, `app_version` (dispatcher routes `version` here), `app_help`, `app_main`, `app_default`, `app_default_print_menu`, `app_default_print_timer`, `app_default_print_selfmgmt`, `app_default_print_running`, `app_default_map_running_pick`, `app_default_run_pick`, `app_default_run_selfmgmt_pick` |
...
When adding a function to `./pomo`:
...
A modular-structure change for pomo is **not done** if any of the following fail:
...
| **TP-TX-01..05** / **TP-TX-08..10** detect + `$PREFIX/tmp` + Git Bash cache helpers (`pomo_apply_target_paths`, `util_ensure_writable_dir`, `util_resolve_volatile_root`, `pomo_require_file`) | `tests/test_cli.sh` | have |
