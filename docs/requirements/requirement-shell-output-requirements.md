...
This requirement is the **project Single Source of Truth** for **all CLI output** of the pomo POSIX shell tool: human messages, machine JSON, channel split (stdout vs stderr), and mode behavior (normal / quiet / JSON / debug).
...
| Exception class | Rule | Live examples in `./pomo` |
|-----------------|------|-----------------------------------|
...
3. **Capture pattern for agents/CI:**  
   `pomo --json <cmd> 2>err.log` → stdout = JSON; stderr = diagnostics as mode allows.  
...
3. Cite `template-*.md` or `skill-*.md` in **product source** (`./pomo`) as output authority — cite this requirement file only.  
...
Output-related work for pomo is **not done** if any of the following fail:
