...
| **L-16** | `sudo pomo` shows 2.1.0 menu; unprivileged `pomo` is `/bin/sh: 0: cannot open /usr/local/bin/pomo: Permission denied` | A `#!/bin/sh` dest needs **other-read** (0755), not execute-only 0711/0700. `sudo` success is not login proof. `chmod +x` is not 0755. Prove **TP-LC-24** (global dest), not only USER_BIN **TP-LC-23**. Gate **CL-ONLINE-INSTALL-SCRIPT** §4. | INC-20260912-001 |
| **L-17** | `sudo pomo self-update` 2.1.0 → 2.2.0 succeeds; then unprivileged `pomo about` is `/bin/sh: 0: cannot open /usr/local/bin/pomo: Permission denied` | Dest mode is applied by the **running** installer, not the downloaded VERSION. 2.1.0 `chmod +x` on mktemp 0600 → **0711**. Shipping `chmod 0755` in 2.2.0 does not protect that upgrade. Prove **TP-LC-25** global dest **0755** after self-update. Repair: root `chmod 0755` or `sudo pomo --force self-update` once 2.2.0 is on disk. | INC-20260915-001 |
