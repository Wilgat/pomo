...
# help must not list CHECKSUM, domain verbs in help, self-uninstall --json
# fail-closed, Type O zero-arg install failure exit, env -u HOME under set -u.
# =============================================================================
...
run_test_cli() {
    ...
    assert_contains "help lists status (domain)" "$_out" "status"
    assert_contains "help lists watch (domain)" "$_out" "watch"
    assert_contains "help lists skip (domain)" "$_out" "skip"
    assert_contains "help lists stop (domain)" "$_out" "stop"
    assert_contains "help lists kill (domain)" "$_out" "kill"
    assert_contains "help lists list (domain)" "$_out" "list"
    assert_contains "help lists stats (domain)" "$_out" "stats"
    assert_contains "help lists theme (domain)" "$_out" "theme"
    assert_contains "help lists --persist" "$_out" "--persist"
    assert_contains "help lists --break" "$_out" "--break"
